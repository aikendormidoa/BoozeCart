...
function renderProducts(list) {
  ...
  btn.addEventListener('click', () => {
    window.location.href = 'cart.html'; // This line redirects
  });
});
}
...
if (checkoutBtn && checkoutModal && closeCheckoutModal && checkoutProducts && checkoutTotal && confirmCheckoutBtn) {
  checkoutBtn.addEventListener('click', function() {
    ...
    // Build product list with quantity and price
    let html = '<ul style="padding-left:0">';
    let total = 0;
    cart.forEach(item => {
      html += `<li style="margin-bottom:8px;">
        <img src="${item.image}" alt="${item.name}" style="width:40px;height:40px;object-fit:cover;vertical-align:middle;border-radius:6px;margin-right:10px;">
  ...
  confirmCheckoutBtn.addEventListener('click', function() {
    ...
    // Optionally, update the cart display and summary
    if (typeof updateCartDisplay === 'function') updateCartDisplay();
    if (typeof updateCartCount === 'function') updateCartCount();
    // Or simply reload the page to reflect the empty cart
    location.reload();
  });
